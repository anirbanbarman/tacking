...
@Component({
selector: 'app-batterytype',
templateUrl: './batterytype.component.html',
styleUrls: ['./batterytype.component.scss']
})
export class BatterytypeComponent implements OnInit,AfterViewInit {
  ...
  getDataList()
  {
    this.dashboardService.getAllbatterytype().subscribe((response:any)=>{
      console.log(response.data);
      //this.displayedColumns=Object.keys(response.data[0])
      //console.log(this.displayedColumns);
      this.displayedColumns = ['id', 'code','make','type','remarks','actions'];
    ...
    save() {
        ...
        if(this.variables.checkCode==false)
        {
          ...
          this.dashboardService.savebatterytype(payload).subscribe((response: any) => {
            if (response && response?.status === 200) {
              this.spinner.hide();
              this.getDataList();
              this.getstatesmaxid();
      ...
      update() {
        ...
        this.dashboardService.updatebatterytype(payload).subscribe((response: any) => {
          if (response && response?.status === 200) {
            this.spinner.hide();
      ...
      delete() {
        ...
        this.dashboardService.deletebatterytype(payload).subscribe((response: any) => {
          if (response && response?.status === 200) {
            this.spinner.hide();
            this.getDataList();
    ...
    getStateData(id:any) {
        ...
        this.dashboardService.getbatterytype(payload).subscribe((response: any) => {
          this.spinner.hide();
      ...
      getbatterytype() {
        this.spinner.show();
        let payload = new FormData();
        payload.append("type","batterytype");
        this.dashboardService.getType(payload).subscribe((response: any) => {
          this.spinner.hide();
          if (response && response.status === 200 && response.data) {
            this.batterytypeList = response.data;
           this.dummybatterytypeList = response.data;

          }
          else {
            const info = response.data;
            console.log('batterytype ->', info);
          }
        }, error => {
          this.spinner.hide();
          failMessage('Something went wrong');
      ...
      checkCode() {
        ...
        this.dashboardService.getbatterytypebycode(payload).subscribe((response: any) => {
          this.spinner.hide();
          if (response && response.status === 200 && response.data) {
            const info = response.data;
